...
func (rh *RewardHandler) reqRewardTransSign(vctx *VerifyContext, bh *types.BlockHeader) {
	...
	consensusLogger.Infof("generate reward for block %v, height=%v, txHash=%v, targetIds=%v,tx=%+v", bh.Hash, bh.Height, reward.TxHash, reward.TargetIds, tx.RawTransaction)
