...
func setupGenesisStateDB(stateDB *account.AccountDB, genesisInfo *types.GenesisInfo) {
	...
	stateDB.SetNonce(AdminAddr(), 2)

	// mining pool and circulates
	stateDB.SetBalance(StakePlatformAddr(), big.NewInt(0).SetUint64(stakePlatformToken))
	stateDB.SetBalance(CirculatesAddr(), big.NewInt(0).SetUint64(circulatesToken))
